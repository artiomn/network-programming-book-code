...
int main()
{
    auto internet_handle = InternetOpen(TEXT("Mozilla/5.12345"), INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, 0);
    ...
    if (internet_handle)
    {
        ...
        if (url_handle)
        {
            ...
            while (InternetReadFile(url_handle, buf.data(), buf.size(), &read_bytes))
            {
                if (!read_bytes) break;
