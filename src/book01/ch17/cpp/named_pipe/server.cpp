...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        if (!ReadFile(
                h_pipe, read_buffer.data(), read_buffer.size() * sizeof(decltype(read_buffer)::value_type), &read_bytes,
                nullptr) &&
        ...
        DWORD written_bytes;
        auto result = WriteFile(h_pipe, new_msg.c_str(), new_msg.size() * sizeof(TCHAR), &written_bytes, nullptr);
