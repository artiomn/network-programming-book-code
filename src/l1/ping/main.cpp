...
public:
    ...
    PacketClass create_response(const typename PacketClass::BufferType &buffer)
    {
        // Skip IP header.
        return PacketClass(buffer.begin() + (reinterpret_cast<const struct ip_hdr*>(buffer.data())->ip_verlen & 0x0f) * sizeof(uint32_t), buffer.end());
    }
...
void send_ping(const socket_wrapper::Socket &sock, const std::string &hostname, const struct sockaddr_in &host_address, const bool ip_headers_enabled = true)
{
    ...
    while(true)
    {
        ...
        auto response = std::move(ping_factory.create_response(buffer));
