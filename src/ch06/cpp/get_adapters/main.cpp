...
int main()
{
    ...
    unsigned long out_size = adapters.size() * sizeof(IP_ADAPTER_ADDRESSES);
    ...
    if (ret_code != NO_ERROR)
    {
        ...
        {
            std::cout << "Increasing buffer [" << adapters.size() << " -> " << out_size / sizeof(IP_ADAPTER_ADDRESSES) + 1 << "]" << std::endl;
    ...
    for (auto adapter = adapters.data(); adapter; adapter = adapter->Next)
    {
        ...
        {
            case IF_TYPE_ETHERNET_CSMACD: std::cout << "Ethernet device\n"; break;
            case IF_TYPE_PPP: std::cout << "PPP network interface\n"; break;
            case IF_TYPE_SOFTWARE_LOOPBACK: std::cout << "Software loopback\n"; break;
            case IF_TYPE_REGULAR_1822: std::cout << "1822\n"; break;
            case IF_TYPE_IEEE80211: std::cout << "Wireless IEEE 802.11\n"; break;
        ...
        switch (adapter->OperStatus)
        {
            ...
            case IfOperStatusTesting: std::cout << "IfOperStatusTesting\n"; break;
            case IfOperStatusUnknown: std::cout << "IfOperStatusUnknown\n"; break;
            case IfOperStatusDormant: std::cout << "IfOperStatusDormant\n"; break;
            case IfOperStatusNotPresent: std::cout << "IfOperStatusNotPresent\n"; break;
            case IfOperStatusLowerLayerDown: std::cout << "IfOperStatusLowerLayerDown\n"; break;
        };
        ...
        for (auto addr = adapter->FirstUnicastAddress; addr; addr = addr->Next)
        {
            ...
            DWORD sz = ip.size() - 1;
            WSAAddressToStringA(
                addr->Address.lpSockaddr, addr->Address.iSockaddrLength, nullptr,
                ip.data(), &sz
            );
