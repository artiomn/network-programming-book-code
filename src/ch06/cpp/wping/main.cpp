...
PICMP_ECHO_REPLY create_reply_buffer(char *buf, unsigned short data_sz, std::vector<char> &reply_holder)
{
...
int get_end_time(const DWORD end_time, const t_ping_data * const wping,
                   const DWORD rtt)
{
...
bool ping(const t_ping_data * const wping)
{
   ...
   auto echo_reply = create_reply_buffer(buf.data(), static_cast<unsigned short>(buf.size()), reply_buf);
   ...
   while (true)
   {
      ...
      if (wping->opts & PING_OPT_VERBOSE)
      {
          std::cout
              ...
              << "\nDelay = " << wping->delay
              << "\nSending to \"" << inet_ntop(AF_INET, &ia_dest.sin_addr, ip_buf, sizeof(ip_buf)) << "\"\n"
              << std::endl;
      }
      ...
      for (int i = 0; i < wping->count; ++i)
      {
         ...
         // Replies count.
         DWORD rep_cnt = IcmpSendEcho(hndlFile, reinterpret_cast<const sockaddr_in*>(&ia_dest)->sin_addr.s_addr,
                                      buf.data(), static_cast<WORD>(buf.size()), &ip_info, echo_reply,
         ...
         if (rep_cnt > 0 && (IP_SUCCESS == echo_reply->Status))
         {
            while (rep_cnt--)
            {
               ...
               {
                   std::cout
                       << "Received from: " << inet_ntop(AF_INET, &echo_reply->Address, ip_buf, sizeof(ip_buf))
         ...
         else
         {
            if (wping->opts & PING_OPT_VERBOSE)
            {
                std::cout
                    << "Host " << wping->host
                    << " (resolved:\"" << inet_ntop(AF_INET, &ia_dest.sin_addr, ip_buf, sizeof(ip_buf)) << "\") doesn't response!"
                    << std::endl;
            }
   ...
   if (!(wping->opts & PING_OPT_AVAIL))
   {
      ...
      {
          std::cout
              << ((t_ping_func::tpf_avg != wping->ping_func) ? end_time : (float)end_time / (good_packets_cnt + 1))
              << std::endl;
