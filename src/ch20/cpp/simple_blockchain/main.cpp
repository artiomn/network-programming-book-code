...
int main()
{
    ...
    server.resource["^/addnode$"]["POST"] =
        [&listOfNodes](shared_ptr<HttpServer::Response> response, shared_ptr<HttpServer::Request> request)
    {
    ...
    server.resource["^/latestchain$"]["GET"] =
        [&bc](shared_ptr<HttpServer::Response> response, shared_ptr<HttpServer::Request> request)
    {
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        {  // add a new block if 2
            ...
            try
            {
                ...
                {
                    printf(
                        "----------------------------------\nPlease join the network... Your blockchain doesn't have "
                        "any blocks ");
                    continue;
                }
                // mine for the has
