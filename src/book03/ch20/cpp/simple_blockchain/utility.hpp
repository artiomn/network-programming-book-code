...
public:
    std::size_t operator()(const std::string &str) const noexcept
    {
        std::size_t h = 0;
        std::hash<int> hash;
        for (auto c : str) h ^= hash(tolower(c)) + 0x9e3779b9 + (h << 6) + (h >> 2);
        return h;
    }
};

using CaseInsensitiveMultimap =
    std::unordered_multimap<std::string, std::string, CaseInsensitiveHash, CaseInsensitiveEqual>;
...
public:
    ...
    static CaseInsensitiveMultimap parse(const std::string &query_string) noexcept
    {
        ...
        if (query_string.empty()) return result;
        ...
        for (std::size_t c = 0; c < query_string.size(); ++c)
        {
            ...
            {
                auto name =
                    query_string.substr(name_pos, (name_end_pos == std::string::npos ? c : name_end_pos) - name_pos);
                if (!name.empty())
                {
                    auto value =
                        value_pos == std::string::npos ? std::string() : query_string.substr(value_pos, c - value_pos);
...
public:
    ...
    static bool parse(
        std::istream &stream, std::string &method, std::string &path, std::string &query_string, std::string &version,
        CaseInsensitiveMultimap &header) noexcept
    {
        ...
        if ((method_end = line.find(' ')) != std::string::npos)
        {
            ...
            if (path_and_query_string_end != std::string::npos)
            {
                ...
                {
                    if (line.compare(
                            path_and_query_string_end + 1, protocol_end - path_and_query_string_end - 1, "HTTP") != 0)
                        return false;
...
public:
    ...
    static bool parse(
        std::istream &stream, std::string &version, std::string &status_code, CaseInsensitiveMultimap &header) noexcept
    {
        header.clear();
        std::string line;
        getline(stream, line);
        ...
        for (std::size_t c = 0; c < line.size(); ++c)
        {
            if (para_start_pos != std::string::npos)
            {
                ...
                else
                {
                    ...
                    {
                        if (line[c] == '"' && c + 1 < line.size()) value_start_pos = c + 1;
                    }
                    else if (line[c] == '"')
                    {
                        result.emplace(
                            line.substr(para_start_pos, para_end_pos - para_start_pos),
                            line.substr(value_start_pos, c - value_start_pos));
                        para_start_pos = std::string::npos;
...
public:
    ...
    {
        long expected = count;
        while (expected >= 0 && !count.compare_exchange_weak(expected, expected + 1)) spin_loop_pause();
