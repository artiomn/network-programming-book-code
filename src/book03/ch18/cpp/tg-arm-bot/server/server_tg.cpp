...
template <>
void create_keyboard<TgBot::InlineKeyboardMarkup>(
    const std::vector<std::vector<std::string>>& button_layout, typename TgBot::InlineKeyboardMarkup::Ptr& kb)
{
    for (auto button_row : button_layout)
    {
        ...
        for (auto button_text : button_row)
        {
            ...
            std::transform(
                cb_data.begin(), cb_data.end(), cb_data.begin(),
                [](unsigned char c) { return std::isspace(c) ? '_' : std::tolower(c); });
            button->callbackData = button_text;
            std::cout << "Key added: " << button_text << " - " << cb_data << std::endl;
...
template <>
void create_keyboard<TgBot::ReplyKeyboardMarkup>(
    const std::vector<std::vector<std::string>>& button_layout, typename TgBot::ReplyKeyboardMarkup::Ptr& kb)
{
    for (auto button_row : button_layout)
    {
...
{
    std::cout << "Run Telegram bot" << std::endl;
...
void create_and_run_tg_bot(const std::string& token, CommandHandler on_command)
{
    ...
    std::cout << "Creating Telegram bot..." << std::endl;
    Bot bot(token);
    ...
    */
    ...
    bot.getEvents().onAnyMessage(
        [&bot, &keyboard, &waiting_command_param, &command, &on_command](Message::Ptr message)
        {
            std::string message_text = message->text;

            std::cout << "User wrote " << message_text << std::endl;
            ...
            else if ("Reboot" == message_text || "Status" == message_text || "Shutdown" == message_text)
            {
                if (!on_command)
                {
                    command.clear();
                ...
                std::transform(
                    message_text.begin(), message_text.end(), message_text.begin(),
                    [](unsigned char c) { return std::isspace(c) ? '_' : std::tolower(c); });

                std::stringstream ss;
                ss << message_text << " executing...";
            ...
            if (waiting_command_param)
            {
                try
                {
                    ...
                    msg << "Command \"";
                    for (const auto& c : command) msg << c << " ";
                    msg << command_param << "\" will be executed...";
