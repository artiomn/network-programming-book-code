...
#include OATPP_CODEGEN_BEGIN(ApiClient)
    ...
    API_CALL(
        "POST", "{api_endpoint}devices", register_device, PATH(String, api_endpoint),
        BODY_DTO(Object<DeviceRegistrationInfoDTO>, registration_info))
    API_CALL("DELETE", "{api_endpoint}/device/{uid}", unregister_device, PATH(String, api_endpoint), PATH(String, uid))
    API_CALL(
        "PUT", "{api_endpoint}device/{uid}", set_device_state, PATH(String, api_endpoint), PATH(String, uid),
